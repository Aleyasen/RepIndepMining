...
ANS_LOC = '/srv/data/projects/apps/RepIndep_CIDR/MATLAB/aale/simtest-freebase/mas-with-citation';
mkdir(ANS_LOC);

%% inputs
node_files = { [DAT_LOC '/paper.txt'], [DAT_LOC '/conf.txt'],[DAT_LOC '/citation.txt'] };
edge_files = { [DAT_LOC '/paper_citation.txt'], [DAT_LOC '/paper_conf.txt'] };
query_file = [DAT_LOC '/_query.txt'];
answer_loc = ANS_LOC;

label = 'conf';
alg = 'modpathsim1'; metapath = {'conf','paper','citation','paper'}; star = [0 0 0 0]; % star = 1 when label at i is star
%alg = 'rwr';
%alg = 'simrank';
%alg = '?';
VERBOSE = 0;
